...
Pl_d0 = x(1); 
n = x(2);
PL_d = Pl_d0 - 10*n*log10(D_win/d_0);
...
var_LSF2 = 1/N_LSF * sum((LSF - mean_LSF).^2);
var_LSF = sqrt(var_LSF2);

CDF_Normal_LSF = (1/2) * (1+erf((LSF-mean_LSF)/(var_LSF*sqrt(2))));
%cdf_nd = 0.5*(1+erf((LSF-mean_LSF)/(variance_LSF*sqrt(2)) ));
